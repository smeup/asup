...
public class RPJDataStructureHelper {
	...
	public static void normalizePositions(QCompoundDataDef<?, QDataTerm<?>> dataStructDef) {
		...
		for (QDataTerm<?> element : listElements) {
			...
			// TODO
			...
			else if (overlay.getPosition().equalsIgnoreCase(Overlay.POS_NEXT)) {
				overlay.setPosition(Overlay.POS_NEXT);
				expectedPosition += bufferedDataDef.getSize();
			} 			
			else if (Integer.parseInt(overlay.getPosition()) == expectedPosition) {
				overlay.setPosition(Overlay.POS_NEXT);
				expectedPosition += bufferedDataDef.getSize();
			} 			
			else if (Integer.parseInt(overlay.getPosition()) > expectedPosition) {
				...
				characterDef.setLength(Integer.parseInt(overlay.getPosition())-expectedPosition);
		...
		for (QDataTerm<?> element : listElements) {
			...
			if(overlay.getPosition() == Overlay.POS_NEXT)
				continue;
								
			Object[] objects = searchOverlayedByPosition(dataStructDef, Integer.parseInt(overlay.getPosition()));
			if (objects == null) {
			...
			if (overlay.getName() != null && !overlay.getName().equals(overlayedTerm.getName())) {
				System.err.println("Unexpected condition: sdiuyf78sdtf8sdf");
				continue;
			}
			...
			if(relativePosition == Integer.parseInt(overlay.getPosition()))
				overlay.setPosition(Overlay.POS_NEXT);
			else {
				overlay.setPosition(new Integer(Integer.parseInt(overlay.getPosition())-relativePosition+1).toString());
			}
		}
	}
	...
	public void writeElements(List<QDataTerm<?>> elements) throws IOException {
		...
		for (QDataTerm<?> element : elements) {
			...
			if (overlay == null) {
				...
				expectedPosition += bufferedDataDef.getSize();
				recalculatedPosition += bufferedDataDef.getLength();

				nameToPosition.put(element.getName(), expectedPosition);
				continue;
			}

			String position = overlay.getPosition();
			if (position.equalsIgnoreCase(Overlay.POS_NEXT) || expectedPosition == Integer.parseInt(position)) {
				...
				if (Integer.parseInt(position) > expectedPosition) {
					// TODO filler
					expectedPosition = Integer.parseInt(position);

					if (expectedPosition < recalculatedPosition)
						System.err.println("Unexpected condition: wer89tvbrybvt87ysdfgs");
					else
						recalculatedPosition = expectedPosition;
				} else {

					QDataTerm<?> overlayedTerm = searchOverlayedByPosition(new ArrayList<QDataTerm<?>>(elements), Integer.parseInt(position));

					if (overlayedTerm == null || (overlay.getName() != null && !overlayedTerm.getName().equals(overlay.getName()))) {
				...
				Integer overlayedExpectedPosition = nameToPosition.get(overlay.getName());
				if (overlayedExpectedPosition == null)
					System.err.println("Unexpected condition: lasdjhgfuysdfczxd");

				if (overlayedExpectedPosition + bufferedDataDef.getSize() >= expectedPosition) {
					expectedPosition = overlayedExpectedPosition;
