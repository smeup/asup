...
public class DataAreaEditor {
	...
	public void setValue(String valore, int initialPosition, int length) throws DataTooLongException, TypeAndValueMismatchException, InvalidBooleanValueException, WrongStartPositionException, InvalidSubstringException {
		if (dataArea.getDataAreaType().equals(DataAreaType.CHARACTER)) {
		    ...
		    valore = new StringBuffer(oldContent)
	           		 .replace(initialPosition - 1, initialPosition -1 + valore.length(), valore)
	                 .toString();
		}
