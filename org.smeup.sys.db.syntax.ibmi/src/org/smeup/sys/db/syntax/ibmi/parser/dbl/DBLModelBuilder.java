...
public class DBLModelBuilder {
	...
	private QBindingStatement manageGetDescriptorStatement(Tree tree) {
		...
		for (int i = 0; i < tree.getChildCount(); i++) {
			...
			case DBLLexer.HEADER_INFO:
				 ...
				 * 
				 * HEADER_INFO ___ VARIABLES ___ VALUE ___ VARIABLE | |__ VALUE
				 * | |___ VALUE ___ VARIABLE |__ VALUE
				 */
	...
	private QBindingStatement manageSetDescriptorStatement(Tree tree) {
		...
		for (int i = 0; i < tree.getChildCount(); i++) {
			...
			case DBLLexer.ITEM_INFO:
				 ...
				 * Expected tree format:
				 * 
				 * ITEM_INFO ____ VARIABLE | |___ ITEMS ______ ITEM _____ NAME |
				 * |__ VALUE __ VARIABLE | |__ ITEM _____ NAME |__ VALUE
				 */
				if (fieldToken.getChildCount() > 0) {
			...
			case DBLLexer.INTO:
				...
				for (int i2 = 0; i2 < fieldToken.getChildCount(); i2++) {
					...
					case DBLLexer.VARIABLE:
						...
						into.setDescriptorName(intoToken.getChild(0).getText());
						break;
					...
					case DBLLexer.USING:
						...
						case DBLLexer.NAMES:
							into.setUsing(UsingType.NAMES);
							break;

						case DBLLexer.LABELS:
							into.setUsing(UsingType.LABELS);
							break;

						case DBLLexer.ANY:
							into.setUsing(UsingType.ANY);
							break;

						case DBLLexer.BOTH:
							into.setUsing(UsingType.BOTH);
							break;

						case DBLLexer.ALL:
							into.setUsing(UsingType.ALL);
							break;

						case DBLLexer.SYSTEM_NAMES:
							into.setUsing(UsingType.SYSTEM_NAMES);
							break;
						}
				...
				describeStatement.setInto(into);
	 ...
	 * 
	 * DECLARE c1 CURSOR FOR (SELECT A, B, C FROM FILE) DECLARE c1 CURSOR FOR
	 * SELECT A, B, C FROM FILE
	 */
