...
drop_index_statement
	:
	DROP INDEX i=table	-> ^(DROP_INDEX_STATEMENT ^(INDEX_NAME $i))
	;
...
create_table_statement
  : CREATE EXTERNAL TABLE t=table def=table_elements USING f=Identifier
    p=param_clause? (LOCATION path=Character_String_Literal)
      -> ^(CREATE_TABLE_STATEMENT ^(TABLE_NAME $t) EXTERNAL ^(TABLE_DEF $def) ^(USING $f) $p?
         ^(LOCATION $path))
  | CREATE TABLE t=table (def=table_elements)? (USING s=Identifier)?
    (p=param_clause)? (AS q=query_expression)?
      -> ^(CREATE_TABLE_STATEMENT ^(TABLE_NAME $t) ^(TABLE_DEF $def)? ^(USING $s)? $p? ^(AS $q)?)
  ;
table_elements
...
create_view_statement
  : 
  CREATE VIEW t=table (col=ordinary_grouping_set)? /*AS q=query_expression*/ -> ^(CREATE_VIEW_STATEMENT ^(VIEW_NAME $t) ^(COLUMNS_LIST $col)? /*^(AS_EXPRESSION $q)*/) 
  ;      
  
create_alias_statement
	: CREATE (or_replace)? ALIAS t=table FOR m=table -> ^(CREATE_ALIAS_STATEMENT (or_replace)? ^(ALIAS_NAME $t) ^(TABLE_NAME $m))
...
drop_alias_statement
	:
	DROP ALIAS t=table -> ^(DROP_ALIAS_STATEMENT ^(ALIAS_NAME $t))	
	;	
...
disconnect_statement
	: DISCONNECT -> ^(DISCONNECT_STATEMENT)
	  |
	  DISCONNECT ALL -> ^(DISCONNECT_STATEMENT ALL)
	  |
...
drop_table_statement
  :
	DROP TABLE t=table  -> ^(DROP_TABLE_STATEMENT ^(TABLE_NAME $t))
	|
	DROP TABLE t=table restrict -> ^(DROP_TABLE_STATEMENT ^(TABLE_NAME $t) restrict)
	|
	DROP TABLE t=table cascade -> ^(DROP_TABLE_STATEMENT ^(TABLE_NAME $t) cascade)
...
drop_view_statement
  :
	DROP VIEW v=table  -> ^(DROP_VIEW_STATEMENT ^(VIEW_NAME $v))
	|
	DROP VIEW v=table restrict -> ^(DROP_VIEW_STATEMENT ^(VIEW_NAME $v) restrict)
	|
	DROP VIEW v=table cascade -> ^(DROP_VIEW_STATEMENT ^(VIEW_NAME $v) cascade)	
  ;    
...
lock_statement
	:
	LOCK TABLE t=table IN SHARE MODE	-> ^(LOCK_TABLE_STATEMENT ^(TABLE_NAME $t) ^(SHARE)) 
	|
	LOCK TABLE t=table IN EXCLUSIVE MODE (allow_read)?	-> ^(LOCK_TABLE_STATEMENT ^(TABLE_NAME $t) ^(EXCLUSIVE) (allow_read)?) 
	;	
...
rename_table_statement
	:
	RENAME TABLE t=table TO n=Identifier FOR SYSTEM NAME s=Identifier -> ^(RENAME_TABLE_STATEMENT ^(TABLE_NAME $t) ^(NEW_NAME $n) ^(SYSTEM $s))
	;	
...
set_connection_statement
	:
	SET CONNECTION d=Identifier -> ^(SET_CONNECTION_STATEMENT ^(DB_NAME $d))
	;	
...
call_procedure
	:
	CALL i=qualified_identifier (LEFT_PAREN funcArgs? RIGHT_PAREN) -> ^(PROCEDURE_CALL_STATEMENT ^(PROCEDURE_NAME $i) ^(PROCEDURE_ARGS funcArgs?))
	;
