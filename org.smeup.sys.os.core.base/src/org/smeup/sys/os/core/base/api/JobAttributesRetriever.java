...
public class JobAttributesRetriever {
	...
	public void main(@DataDef(length = 10) QCharacter cLVarForJOB10, @DataDef(length = 10) QCharacter cLVarForUSER10, @DataDef(length = 6) QCharacter cLVarForNBR6,
			...
			@Unsupported @DataDef(precision = 2) QDecimal cLVarForLOGSEV20, @Unsupported @DataDef(length = 10) QCharacter cLVarForLOGTYPE10,
			@Unsupported @DataDef(length = 10) QCharacter cLVarForLOGCLPGM10, @Unsupported @DataDef(length = 10) QCharacter cLVarForLOGOUTPUT10,
			@Unsupported @DataDef(precision = 2) QDecimal cLVarForJOBMSGQMX20, @Unsupported @DataDef(length = 10) QCharacter cLVarForJOBMSGQFL10,
			...
			@Unsupported @DataDef(length = 13) QCharacter cLVarForDEVRCYACN13, @ToDo @DataDef(precision = 5) QDecimal cLVarForRTNCDE50, @ToDo @DataDef(length = 1) QCharacter cLVarForENDSTS1,
			...
			@ToDo @DataDef(length = 2) QCharacter cLVarForCNTRYID2, @ToDo @DataDef(precision = 5) QDecimal cLVarForCCSID50, @ToDo @DataDef(precision = 5) QDecimal cLVarForDFTCCSID50,
			...
			@ToDo @DataDef(precision = 3) QDecimal cLVarForTIMOFFSET30, @ToDo @DataDef(precision = 3) QDecimal cLVarForYEAROFS30, @ToDo @DataDef(length = 15) QCharacter cLVarForACGCDE15,
			@ToDo @DataDef(length = 8) QCharacter cLVarForSWS8, @ToDo @DataDef(precision = 2) QDecimal cLVarForRUNPTY20, @ToDo @DataDef(precision = 7) QDecimal cLVarForTIMESLICE70,
			@ToDo @DataDef(length = 10) QCharacter cLVarForPURGE10, @ToDo @DataDef(precision = 7) QDecimal cLVarForDFTWAIT70, @ToDo @DataDef(length = 10) QCharacter cLVarForSBMMSGQ10,
