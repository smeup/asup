...
public @Supported class MessageRetriever {
	...
	public void main(
			...
			@Supported @DataDef(precision = 2) QDecimal cLVarForSEV20,
			@Unsupported @DataDef(length = 9) QCharacter cLVarForALROPT9,
			@Unsupported @DataDef(length = 1) QCharacter cLVarForLOGPRB1,
			@Unsupported @DataDef(binaryType = BinaryType.INTEGER) QEnum<CONVERTTOCCSIDEnum, QBinary> convertToCCSID,
			@Unsupported @DataDef(binaryType = BinaryType.INTEGER) QEnum<MESSAGEDATACCSIDEnum, QBinary> messageDataCCSID,
			@Unsupported @DataDef(precision = 5) QDecimal cLVarForTextCCSID50,
			@Unsupported @DataDef(precision = 5) QDecimal cLVarForDataCCSID50) {
		
		
		OperatingSystemMessageException msgException = 
				exceptionManager.prepareException(job, 
												  messageIdentifier.trimR(), 
												  messageFile.name.trimR(), 
